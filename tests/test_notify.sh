...
now=$(date +%Y-%m-%dT%H:%M:%S)
{
  ...
  printf '%s\tstop\theartbeat\tprojC\tsess3\ttty:/dev/ttys3\tbusy\n' "$now"
...
assert_contains "$out" "actionable 3" "counts actionable (notified) deliveries"
assert_contains "$out" "heartbeat 1" "heartbeat count is visible in the headline, not silently dropped"
...
assert_contains "$out" "Click-through: 1 of 3 actionable" "click-through rate is explicitly scoped to actionable deliveries"
assert_contains "$out" "Bursts: 3" "bursts count actionable AND heartbeat pings (both are real notifications landing on the user's Mac)"
